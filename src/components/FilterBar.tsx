...
export function FilterBar({
  ...
  const makesLabel  = filters.make ? filters.make : "Make";
  const yearLabel   = filters.yearMin || filters.yearMax ? `${filters.yearMin ?? "Any"}–${filters.yearMax ?? "Any"}` : "Year";
  const priceLabel  = PRICE_BAND_LABEL[filters.priceBand];
  const fuelLabel   = filters.fuel ?? "Fuel";
  const transLabel  = filters.transmission ?? "Transmission";
  const sortLabel   = SORT_LABEL[filters.sort];
  ...
  return (
    <View>
      ...
      <BottomSheet open={openSheet === "make"} title="Make" onClose={close}>
        <PickerList
          options={[{ value: null, label: "Any make" }, ...availableMakes.map((m) => ({ value: m, label: m }))]}
          value={filters.make}
          onSelect={(v) => { update({ make: v }); close(); }}
        />
      </BottomSheet>

      <BottomSheet open={openSheet === "year"} title="Year range" onClose={close}>
        <YearRangePicker
          min={filters.yearMin}
          max={filters.yearMax}
      ...
      <BottomSheet open={openSheet === "price"} title="Price range" onClose={close}>
        <PickerList
          options={[
            { value: "all",     label: PRICE_BAND_LABEL.all     },
            { value: "u50",     label: PRICE_BAND_LABEL.u50     },
            { value: "50-100",  label: PRICE_BAND_LABEL["50-100"] },
            { value: "100-150", label: PRICE_BAND_LABEL["100-150"] },
            { value: "150p",    label: PRICE_BAND_LABEL["150p"] },
          ]}
      ...
      <BottomSheet open={openSheet === "fuel"} title="Fuel type" onClose={close}>
        <PickerList
          options={[{ value: null, label: "Any fuel" }, ...FUEL_OPTIONS.map((f) => ({ value: f, label: f }))]}
          value={filters.fuel}
      ...
      <BottomSheet open={openSheet === "trans"} title="Transmission" onClose={close}>
        <PickerList
          options={[{ value: null, label: "Any" }, ...TRANSMISSION_OPTIONS.map((tr) => ({ value: tr, label: tr }))]}
      ...
      <BottomSheet open={openSheet === "sort"} title="Sort by" onClose={close}>
        <PickerList
...
function YearRangePicker({
  ...
  return (
    <View style={{ paddingBottom: 24 }}>
      <Text style={styles.fieldHelp}>Filter vehicles by model year — leave either blank to leave it open.</Text>
      ...
      {invalid && <Text style={styles.fieldError}>From year must be ≤ To year.</Text>}
